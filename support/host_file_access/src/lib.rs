...
impl<'a, T: Read + Write> Drop for HostFileAccess<'a, T> {
    fn drop(&mut self) {
        tracing::debug!("Dropping HostFileAccess, sending EOF header");
...
impl<'a, T: Read + Write> Write for HostFileAccess<'a, T> {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        tracing::debug!("Writing header to host file access: {:?}", header);
        self.transport.write_all(header.as_bytes())?;

        tracing::debug!("Writing data to host file access");
...
impl<'a, T: Read + Write> Read for HostFileAccess<'a, T> {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        tracing::debug!("Reading size from host file access");
        let mut size = 0usize;
        self.transport
            .read_exact(size.as_mut_bytes())
            .map_err(std::io::Error::other)?;
        if size == 0 {
            tracing::debug!("End of file reached during read operation");
            return Ok(0);
        }

        tracing::debug!("Reading data from host file access");
...
mod tests {
    ...
    fn test_test_loopback() {
        ...
        let host_thread = std::thread::spawn(|| {
            ...
            let mut buf = vec![];
            let size = cursor.read_to_end(&mut buf).unwrap();
            assert_eq!(&buf[..size], DATA);

            buf.clear();
            let size = cursor.read_to_end(&mut buf).unwrap();
            assert_eq!(size, 0, "Expected no more data after EOF");
        });
